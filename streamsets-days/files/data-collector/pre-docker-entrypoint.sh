...
if [ -z "${SDC_INSTALL_SAMPLE_PIPELINES}" ]
then
  echo "INFO: No sample pipelines to install.  SDC_INSTALL_SAMPLE_PIPELINES not configured."
elif [ ! -d "${SDC_INSTALL_SAMPLE_PIPELINES}" ]
then
  echo "ERROR: SDC_INSTALL_SAMPLE_PIPELINES did not specify a folder.  SDC_INSTALL_SAMPLE_PIPELINES=${SDC_INSTALL_SAMPLE_PIPELINES}"
else
  echo "Installing sample pipelines ${SDC_INSTALL_SAMPLE_PIPELINES}/*"
  # Recurse.  And do not copy if the file is older.
  cp -r -u "${SDC_INSTALL_SAMPLE_PIPELINES}"/* "${SDC_DIST}/samplePipelines/"
fi
